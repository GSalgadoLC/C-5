 ...
 * Purpose: Project 2 BATTLESHIP V6
 */
...
void initBoard(string [][columns],string [][columns]);                                             //Initializes the 2d array board to 10x10 with empty string "-"    
...
int main(int argc, char** argv) {
    ...
    initBoard(board,gBD);                                                           //Calls our initialization board function, to start the game                                          
    ...
    int numCor = 0;
    float prcntC = 0.00;
    do{                                                                         //Do while loop until the game ends
        numGues += 1;
        char cRow;
        int guesRow;
        int guesCol;                                                            //Variables to store the users guesses
        printBoard(gBD, topBar,leftBar);
        cout<<"\n\tEnter the row....\t";
        cin>>cRow;
        cout<<"\n\tEnter the column...\t";                                      //Prompts user for input
        cin>>guesCol;
        guesCol = guesCol -1 ;
        
        switch(cRow){
            case 'A':
                guesRow = 0;                                                    //Switch statment to covert character input A-J to index row value correctly
                break;
            case 'B':
                guesRow = 1;
                break;
        ...
        if (board[guesRow][guesCol] == boat) {
            cout << "\n\n\t\tBOOM...YOU HIT A SHIP!" << endl;                   //IF conditional to test if user has correctly guessed 
            board[guesRow][guesCol] = hit;                                      //Marks shipped hit "X"
            gBD[guesRow][guesCol] = hit;
            numCor +=1;
        } 
        else {
            cout << "\n\n\t\tMISS, YOU MISSED." << endl;                        //IF not hit then missed
            board[guesRow][guesCol] = miss;                                     //Marks board as missed
            gBD[guesRow][guesCol] = miss;
        }
        
        int count = 0;
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < columns; j++) {                                 //To determine if the ship are sunk or not
                if (board[i][j] == boat) {                                      //Loops through board and checks    
                    count++;
                }
            }
        }
        if (count == 0) {
            cout << "\n\tYOU HAVE SUNK ALL SHIPS!!!" << endl;                   //Displays if all ships are hit
            cout << "\n\tYOU HAVE SUNK ALL SHIPS!!!" << endl;
            cout << "\n\tYOU HAVE SUNK ALL SHIPS!!!" << endl;
            
            cout<<"\n\n\n\tYOU GUESSED "<<setw(10)<<numGues<<" Times"<<endl;
            cout<<"\n\n\tYOU GUESSED CORRECTLY "<<setw(10)<<numCor<<" Times"<<endl;           
            prcntC = numCor/numGues;
            cout<<"\n\n\tYOU GOT "<<setw(10)<<prcntC<<" %100 right"<<endl;
...
void disRules(){
    ...
    cout<<"\n\t\t\t\t- (1) BattleShip 4 Holes,(1) Cruiser 3 Holes,";  //Rule prompt
    cout<<"\n\t\t\t\t- (2) Submarines 2 Holes and (2) Destroyers 1 Hole";                 //Rule prompt
    cout<<"\n\t\t\t+ Each ship will be placed horizontally or vertically";//Rule prompt
    cout<<"\n\t\t\t+ Ships cannot overlap each other but can be side by side";   //Rule prompt
    cout<<"\n\t\t\t+ Ships will not change positions";  //Rule prompt
...
void initBoard(string board[][columns],string gBD[][columns]){                                        //Function initializing our 2d board with placeholder character value "-"
